<?php
/**
 * modules/fertilizer/stock.php
 * Fertilizer stock movements — receive, issue and adjust.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($productId <= 0 || !in_array($type, ['in', 'out', 'adjustment'], true) || $qty == 0) {
        ...
        redirect('modules/fertilizer/stock.php');
    }

    $prod = $pdo->prepare('SELECT cost_per_unit FROM fertilizer_products WHERE id = ?');
    $prod->execute([$productId]);
    $cost = (float)($prod->fetchColumn() ?: 0);

    record_stock_movement($pdo, 'fertilizer_products', 'fertilizer_stock_movements', $productId, $type, $qty, [
        'movement_date' => $date,
        'unit_cost'     => $type === 'in' ? $cost : null,
        'reference_type' => 'manual',
        'remarks'       => $remarks,
    ]);
    log_activity($pdo, current_user_id(), $type, 'fertilizer', "Stock $type qty $qty for product #$productId");
...
$total = (int)$pdo->query('SELECT COUNT(*) FROM fertilizer_stock_movements')->fetchColumn();
$moves = $pdo->query(
    "SELECT m.*, p.product_name, p.unit FROM fertilizer_stock_movements m
     JOIN fertilizer_products p ON p.id = m.product_id
     ORDER BY m.movement_date DESC, m.id DESC LIMIT $offset, " . PER_PAGE
)->fetchAll();

$canManage = can('fertilizer.manage');
$page_title = 'Fertilizer Stock';
...
<div class="card">
    <div class="card-body">
        ...
        <?= render_pagination($total, PER_PAGE, $page, 'modules/fertilizer/stock.php') ?>
    </div>
</div>
